...
The first bundled detector is `pot_yolo_int8.tflite`, an int8 TensorFlow Lite YOLO detector trained for `pothole` on RDD2022-derived road-damage data. Android stores `pot-yolo-int8-780aff5` as the detector model version on every queued candidate, and uses a `0.65` confidence threshold before showing or queuing a match.
